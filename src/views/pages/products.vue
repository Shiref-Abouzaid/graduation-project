<template>
    <div class="d-flex align-end">
      ...
      <v-btn class="mb-2" color="primary" size="large" to="/products/add">
        <PlusOutlined />
        Add Product</v-btn>
    </div>
    <UiParentCard title="Products" bg="white">
          ...
          <tr v-if="!isLoading" v-for="item in items" :key="item.id">
            ...
            <td>
              <div class="actions-container d-flex">
                <v-btn color="info" icon title="view" :to="`/products/${item.id}`"
                  ><EyeOutlined
                /></v-btn>
                <v-btn
                  color="success"
                  ...
                  icon
                  title="edit"
                  :to="`/products/add?editMode=true&id=${item.id}`"
                  ><FormOutlined
                /></v-btn>
  ...
  function deleteProduct() {
    ...
    fetchWrapper.delete(`/Product/${idToDelete.value}`).then(() => {
